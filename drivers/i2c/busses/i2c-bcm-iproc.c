...
static int bcm_iproc_i2c_cfg_speed(struct bcm_iproc_i2c_dev *iproc_i2c)
{
	unsigned int bus_speed;
	u32 val;
	int ret = device_property_read_u32(iproc_i2c->device,
					   "clock-frequency", &bus_speed);
	if (ret) {
		dev_info(iproc_i2c->device,
			"unable to interpret clock-frequency property\n");
		bus_speed = 100000;
	}
...
#ifdef CONFIG_ACPI
static const struct acpi_device_id bcm_iproc_i2c_acpi_match[] = {
	{ "BCMI2C00", 0 },
	{ }
};
MODULE_DEVICE_TABLE(acpi, bcm_iproc_i2c_acpi_match);
#endif
...
static struct platform_driver bcm_iproc_i2c_driver = {
	.driver = {
		...
		.acpi_match_table = ACPI_PTR(bcm_iproc_i2c_acpi_match),
