...
static int brcmnand_init_cs(struct brcmnand_host *host)
{
	...
	if (of_property_read_bool(dn, "flash-removable"))
		ctrl->soc->quirks |= BRCMNAND_QUIRK_FLASH_REMOVABLE;
	/*
	 * flash removable qruick is used for pegasus where nand flash is
	 * plugged via daughter card and if that is not connected, then do not
	 * proceed with further initialization.
	 */
	if (ctrl->soc->quirks & BRCMNAND_QUIRK_FLASH_REMOVABLE) {
		intfc = brcmnand_read_reg(ctrl, BRCMNAND_INTFC_STATUS);
		if (!(intfc & INTFC_CTLR_READY)) {
			dev_err(&pdev->dev, "interface not ready: intfc:0x%x\n",
				intfc);
			return -ENXIO;
		}
	}
