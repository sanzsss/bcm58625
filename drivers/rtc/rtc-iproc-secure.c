...
static int bcm_rtc_suspend(struct device *dev)
{
	return 0;
}

static int bcm_rtc_resume(struct device *dev)
{
	int ret = 0;

	ret = bbl_init();
	if (ret < 0) {
		dev_err(dev, "Unable to initialize bbl err=%d\n", ret);
		return ret;
	}
	ret = bcm_rtc_enable();
	if (ret < 0) {
		dev_err(dev, "couldn't enable rtc in resume err=%d\n", ret);
		return ret;
	}
...
static SIMPLE_DEV_PM_OPS(bcm_rtc_pm_ops, bcm_rtc_suspend, bcm_rtc_resume);
