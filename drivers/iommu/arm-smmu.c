...
#define sCR0_FB				(1 << 13)
#define sCR0_MTCFG			(1 << 20)
#define sCR0_BSU_SHIFT			14
#define sCR0_MEMATTR_SHIFT		16
#define sCR0_SHCFG_SHIFT		22
#define sCR0_MEMATTR_MASK		0xF
...
#define ARM_SMMU_MASTER_QUIRK_INST_AS_DATA	(1 << 0)
	int				quirks;
};
				      ...
				      struct arm_smmu_master_cfg *cfg)
{
	...
	for (i = 0; i < cfg->num_streamids; ++i) {
		...
		if ((smmu->version == ARM_SMMU_V2) &&
		    (cfg->quirks & ARM_SMMU_MASTER_QUIRK_INST_AS_DATA))
...
static int arm_smmu_attach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	if (of_property_read_bool(dev->of_node, "smmu-inst-as-data"))
		cfg->quirks |= ARM_SMMU_MASTER_QUIRK_INST_AS_DATA;
...
static void arm_smmu_device_reset(struct arm_smmu_device *smmu)
{
	...
	if (of_property_read_u32(smmu->dev->of_node, "sharability",
				 &shcfg) == 0) {
		dev_info(smmu->dev, "Override default shcfg for bypass transactions = 0x%x\n",
			 shcfg);
		/* SHCFG value */;
		reg &= ~(sCR0_SHCFG_MASK << sCR0_SHCFG_SHIFT);
		reg |= (shcfg << sCR0_SHCFG_SHIFT);
	}

	if (of_property_read_u32(smmu->dev->of_node, "memattr",
				 &memattr) == 0) {
		dev_info(smmu->dev, "Override default memattr for bypass transactions = 0x%x\n",
			 memattr);
		/* MTCFG to override */
		reg |= sCR0_MTCFG;
		/* MEMATTR value */;
		reg &= ~(sCR0_MEMATTR_MASK << sCR0_MEMATTR_SHIFT);
		reg |= (memattr << sCR0_MEMATTR_SHIFT);
	}
