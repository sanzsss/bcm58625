...
#ifdef CONFIG_ARM64
static ssize_t
exception_show(struct device *dev,
	       struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", get_exception_value());
}
...
static ssize_t
exception_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	unsigned long val;
	...
	if (kstrtoul(buf, 0, &val) < 0)
		return -EINVAL;
	if (val)
		enable_exception("pci");
	else
		disable_exception("pci");
	return count;
}
static DEVICE_ATTR_RW(exception);
#endif
