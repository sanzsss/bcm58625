...
struct pdc_state {
	...
	/* Width of DMA addressable space */
	u32 dma_addr_width;
...
static int pdc_dt_read(struct platform_device *pdev, struct pdc_state *pdcs)
{
	...
	err = of_property_read_u32(dn, "brcm,dma-addr-width",
				   &pdcs->dma_addr_width);
	if (err < 0)
		dev_err(dev,
			"%s failed to get DMA address width from device tree",
			__func__);

	pdcs->use_bcm_hdr = of_property_read_bool(dn, "brcm,use-bcm-hdr");

	if (of_device_is_compatible(dn, "brcm,iproc-fa2-mbox"))
		pdcs->hw_type = FA_HW;
	else
		pdcs->hw_type = PDC_HW;
...
static int pdc_probe(struct platform_device *pdev)
{
	...
	err = dma_set_mask_and_coherent(dev,
					DMA_BIT_MASK(pdcs->dma_addr_width));
	if (err) {
